...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if ((arg == "-o" || arg == "--url") && i + 1 < argc) pool_url = argv[++i];
        else if ((arg == "-u" || arg == "--user") && i + 1 < argc) user = argv[++i];
        else if ((arg == "-p" || arg == "--pass") && i + 1 < argc) pass = argv[++i];
        else if ((arg == "-d" || arg == "--device") && i + 1 < argc) device_id = std::stoi(argv[++i]);
        else if ((arg == "-b" || arg == "--block-size") && i + 1 < argc) block_size = (uint32_t)std::stoi(argv[++i]);
    }
